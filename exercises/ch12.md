...
1. DB 구현 단계의 주요 업무가 아닌 것은? `1`
   1. DB에 초기 데이터를 삽입한다. `데이터삽입 → "운영단계`
...
(100, '가상실험', '2공학관', 'B203', 'comp'),
(110, '인공지능', '2공학관', 'A101', 'comp');
...
8. 02번 문제에서 생성한 1ab 태이블을 기초로 하여 소속학과 id가 ‘comp’인 모든 실험실의 명칭 (name)， 호실셀(room id)，면적 (lab_size)을 포함하는 com_lab_view라는 뷰를 생성하는 SQL 문을 작성하시오.
```sql
...
9. 02번 문제에서 생성한 lab 태이블의 기본키인 실험실번호(lab_num) 값을 자동으로 생성하는 데 사용할 시뭔스인 1ab_num_seq를 생성하려고 한다. 첫 번째 시퀀스 값은 120이고， 최대 990까지 10씩 증가시키고， 최댓값에 도달한 후에 다시 첫 번째 시퀀스 번호부터 생성하지 않고， 시퀀스 값을미리 생성하지 않도록 시퀀스를 정의하는 SQL 문을작성하시오.
```sql
CREATE SEQUENECE lab_num_seq
START WITH 120
INCREMENT BY 10
MAXVALUE 990
NOCYCLE
...
```sql
INSERT INTO lab (lab_num, name, building, room_id, dept_id) VALUES
(lab_num_seq.NEXTVAL, '네트워크', '2공학관', 'B281', 'comp')
```
